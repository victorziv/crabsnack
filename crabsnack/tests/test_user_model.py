...
class TestUserModelTestCase:
    ...
    def setup_class(cls):
        ...
        Role.set_query()
        Role.insert_roles()
        User().save_user(email='pablo@nowhere.com', username='pablo', password=cls.password)
    ...
    def test_password_setter(self):
        u = User().save_user(email='test3@nowhere.com', username='test3', password=self.password)
    ...
    def test_password_salts_are_random(self):
        u1 = User().save_user(email='test1@nowhere.com', username='test1', password=self.password)
        u2 = User().save_user(email='test2@nowhere.com', username='test2', password=self.password)
        assert u1.password_hash != u2.password_hash
    # ______________________________

    def test_roles_and_permissions(self):
        u = User().save_user(email='test4@nowhere.com', username='test4', password='test44')
    ...
    def test_user_confirmed(self):
        ...
        u.query.update_confirmed(u.id, True)
        u = User().get_by_field(name='id',value=u.id)
        assert u.confirmed
    # ______________________________
